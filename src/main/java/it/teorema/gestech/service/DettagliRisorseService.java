...
public interface DettagliRisorseService extends JpaRepository <DettagliRisorse, Integer> {
	...
	@Query("select r.id, r.nomeCognome, a.nome "
			+ "from Risorse r, AziendeRisorse ar, Aziende a "
			+ "where r.id = ar.idRisorsa and a.id = ar.idAzienda ")
	List<Object> allDipendenti();
	
	@Modifying
	@Transactional
	@Query("delete from DettagliRisorse where idRisorsa = :idRisorsa")
	void deleteByIdCandidato(int idRisorsa);
	...
	@Query("from DettagliRisorse where idRisorsa = :idCandidato")
	DettagliRisorse findByIdRisorsa(int idCandidato);
}